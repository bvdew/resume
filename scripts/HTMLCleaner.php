...
class HTMLCleaner {
	...
	public function cleanUpHTML($htmlCode) {
		...
		$cleanHtmlCode = $this->cleanUpStrongTagsIfAny($cleanHtmlCode);
	...
	private function cleanUpStrongTagsIfAny($input) {
		$output = preg_replace("/(<strong[a-zA-Z0-9_.=,:;#'\"\- \(\)]*>)/mi", "<b>", $input);
		$output = preg_replace("/(<\/strong>)/mi", "</b>", $output);
		return $output;
	}
